...
void D2Q9_cpu::macro(LBMFieldCPU& field) {
    int nx = field.nx; 
    int ny = field.ny;
    ...
    for(int i = 0; i < ny; ++i){
        for(int j = 0; j < nx; ++j){
            ...
            int flat = i*nx + j;
            if(rho_ij < 1e-6f) rho_ij = 1e-6f; // 防止除零
...
void D2Q9_cpu::collide(LBMFieldCPU& field) {
    int nx = field.nx;
    int ny = field.ny;
    ...
    for(int i = 0;i < ny; ++i) {
        for(int j = 0;j < nx; ++j) {
            ...
            if(field.is_cylinder[flat]){
                for(int k=0;k<Q;k++) {
                    field.f_next[index(j, i, k, nx, ny, field.layout)] = field.f[index(j, i, k, nx, ny, field.layout)];
                }
                continue;
            }
            ...
            for(int k = 0; k < Q; ++k) {
                ...
                if(std::isnan(field.f_next[id])) field.f_next[id] = feq;  // 防止 NaN
            }
        }
    }
}

void D2Q9_cpu::stream(LBMFieldCPU& field) {
    int nx = field.nx;
    int ny = field.ny;
...
void D2Q9_cpu::bounce_back(LBMFieldCPU& field) {
    int nx = field.nx;
    int ny = field.ny;
    ...
    for(int x = 0; x < nx; ++x) {
        ...
        auto swap_dirs = [&](int y, int a, int b){ 
            std::swap(field.f[index(x, y, a, nx, ny, field.layout)], field.f[index(x, y, b, nx, ny, field.layout)]);
        };
...
void D2Q9_cpu::initialize(LBMFieldCPU& field) {
    int nx = field.nx;
    int ny = field.ny;
    int cx0 = field.nx / 4;
    int cy0 = field.ny / 2;
    ...
    for(int y = 0; y < ny; ++y) {
        for(int x = 0; x < nx; ++x) {
            ...
            float ux = D2Q9_cpu::ux0;
            float uy = 0.f;
            float rho = D2Q9_cpu::rho0;
